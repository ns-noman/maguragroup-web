						...
						@foreach($data['companies'] as $key => $company)
							@if($key==0 || ceil(count($data['companies'])/2) == $key)
								<div class="col-md-6">
									<ul class="list-group">
							@endif
										<li class="bg-info list-group-item d-flex align-items-center shadow-sm p-3 mb-3 hover-shadow">
											<a href="{{ $company['site_link'] }}" class="d-flex align-items-center text-decoration-none">
												...
												<h3 style="font-size: {{ strlen($company['title']) > 40 ? 20 : 26 }}px;">{{ $company['title'] }}</h3>
											</a>
										</li>
